...
function Text({ children, size = 1, left, right, top, bottom, color = "white", opacity = 1, height = 0.01, layers = 0, font = "/diamonds/MOONGET_Heavy.blob", ...props }) {
  const data = useLoader(FontLoader, "/diamonds/MOONGET_Heavy.blob")
  const geom = useAsset(() => new Promise((res) => res(new THREE.TextBufferGeometry(children, { font: data, size: 1, height, curveSegments: 32 }))), [children])
